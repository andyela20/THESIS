...
export default function CrystalLibrary({
  ...
  const fetchAnalyses = async () => {
    try {
      ...
      setCrystalRecords(Array.isArray(data) ? data : []);
    } catch (err) {
  ...
  const handleDeleteGroup = async (group) => {
    ...
    try {
      await Promise.all(group.crystals.map(c => deleteAnalysis(c._id)));
      setCrystalRecords(prev => prev.filter(r => r.patientId !== group.patientId));
    } catch (err) {
  ...
  const handleExportPDF = (group) => {
    ...
    const totalCrystals = group.crystals.reduce((sum, c) => sum + c.count, 0);

    const rows = group.crystals.map(c => {
      ...
      return `
        <tr>
          <td style="padding:10px 14px;border-bottom:1px solid #EEEFE8;font-size:12px;color:#141514;">
            <span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${CRYSTAL_COLORS[c.crystalType] || '#888'};margin-right:8px;vertical-align:middle;"></span>
            ${c.crystalType}
          </td>
    ...
    const html = `
      <html>
      <head>
        <title>CrystalScope Report — ${group.patientName}</title>
        <style>
      ...
      <body>
        ...
        <div class="info">
          ...
          <div class="box"><div class="lbl">Total crystals</div><div class="val">${totalCrystals}</div></div>
        </div>
        <table>
          <thead><tr><th>Crystal type</th><th>Count</th><th>Distribution</th><th>Risk</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </body>
      </html>
      ...
      <div style={styles.body}>
        ...
        <div style={styles.main}>
          <div style={styles.pane}>
            ...
            {loading ? (
              <div style={styles.loadingWrap}>Loading crystal library...</div>
            ) : groupList.length === 0 ? (
                    ...
                    {group.crystals.map((crystal, i) => (
                      <div key={i} style={styles.crystalCell}>
                        <div style={styles.cellTop}>
                          ...
                          <span style={{ ...styles.riskBadge, ...( RISK_STYLE[crystal.risk] || RISK_STYLE.Low) }}>
                            {crystal.risk}
                          </span>
                        </div>
...
const styles = {
  ...
  pill:        { padding: '6px 16px', borderRadius: '100px', border: '1.5px solid #D8DAD0', background: '#fff', fontSize: '12px', fontWeight: 500, cursor: 'pointer', fontFamily: "'Poppins', sans-serif" },
  ...
  exportBtn:   { fontSize: '11px', fontWeight: 600, padding: '5px 12px', borderRadius: '8px', border: '1px solid #D8DAD0', background: '#fff', color: '#1F5330', cursor: 'pointer', fontFamily: "'Poppins', sans-serif" },
  crystalGrid: { display: 'grid', gap: '1px', background: '#EEEFE8' },
  crystalCell: { background: '#fff', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '4px' },
  cellTop:     { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' },
