...
export default function ParticleLibrary({
  ...
  const handleDeleteGroup = async (group) => {
    ...
    if (!window.confirm(`Delete this analysis for ${group.patientName} (${dateText})?`)) return;

    try {
      await Promise.all(group.particles.map(c => deleteAnalysis(c._id)));

      const idsToDelete = new Set(group.particles.map(c => c._id));
      setParticleRecords(prev => prev.filter(r => !idsToDelete.has(r._id)));
    } catch (err) {
      console.error('Error deleting analysis group:', err);
  ...
  /*
    IMPORTANT FIX:
    Old behavior grouped only by patientId.
    That caused multiple analyses from the same patient to merge into one panel.

    New behavior groups by patient + sample/analysis/date.
    This makes each saved analysis appear as its own separate panel.
  */
  const grouped = filtered.reduce((acc, record) => {
    const analysisDate = record.date || record.createdAt || record.updatedAt || '';
    ...
    const key = `${record.patientId || 'unknown-patient'}-${analysisKey}`;
  ...
  const handleExportPDF = (group) => {
    ...
    const total = group.particles.reduce((sum, c) => sum + Number(c.count || 0), 0);
    ...
    const rows = group.particles.map(c => {
      ...
      return `
        <tr>
          ...
          <td style="padding:10px 14px;border-bottom:1px solid #EEEFE8;font-size:12px;">${count}</td>
    ...
    const html = `
      ...
      <body>
        ...
        <div class="sub">Generated on ${new Date().toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        })}</div>
        ...
        <div class="info">
          <div class="box"><div class="lbl">Patient</div><div class="val">${group.patientName || 'Unknown'}</div></div>
          <div class="box"><div class="lbl">Patient ID</div><div class="val">${group.patientId || 'N/A'}</div></div>
          <div class="box"><div class="lbl">Sample ID</div><div class="val">${group.sampleId || 'N/A'}</div></div>
          ...
          <div style={styles.pane}>
            <div style={styles.header}>
              ...
              <div style={styles.entryCount}>
                {groupList.length} {groupList.length === 1 ? 'analysis' : 'analyses'} · {totalEntries} {totalEntries === 1 ? 'entry' : 'entries'}
              </div>
            </div>
            ...
            <div style={styles.filters}>
              {PARTICLE_GROUPS.map(group => (
                <button
                  ...
                  style={{
                    ...styles.pill,
                    ...(activeGroup === group ? styles.pillActive : {}),
                  }}
                  onClick={() => {
              ...
              <div style={styles.subFilters}>
                {availableTypes.map(type => (
                  <button
                    ...
                    style={{
                      ...styles.subPill,
                      ...(activeFilter === type ? styles.subPillActive : {}),
                    }}
              ...
              groupList.map((group) => (
                <div key={group.groupId} style={styles.group}>
                  <div style={styles.groupHeader}>
                    ...
                    <div>
                      ...
                      <div style={styles.groupMeta}>
                        {group.patientId || 'No patient ID'} · {group.sampleId || 'No sample ID'} ·{' '}
                        {group.date
                          ? new Date(group.date).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'short',
                  ...
                  <div
                    style={{
                      ...styles.particleGrid,
                      gridTemplateColumns: getGridCols(group.particles.length),
                    }}
                  >
                    {group.particles.map((particle, i) => {
                      ...
                      return (
                        <div key={`${particle._id || particle.particleType}-${i}`} style={styles.particleCell}>
